...
import CommSkillsQuestionCard, { type CommSkillsAnswerRecord } from "./CommSkillsQuestionCard";
...
import { commSkillsTopics, type CommSkillsSource } from "@/lib/commSkills/types";
import { trackInteract } from "@/lib/track";

const allBank = [...commSkillsPastQuestions, ...commSkillsConceptQuestions];
const allTopicIds = commSkillsTopics.map((t) => t.id);

type SourceFilter = "all" | CommSkillsSource;

const sourceFilters: { id: SourceFilter; label: string }[] = [
  { id: "all", label: "Both sections" },
  { id: "past-question", label: "Section A — Past Questions" },
  { id: "lecture-material", label: "Section B — Lecturer Material" },
];

export default function CommSkillsRoom() {
  const [sourceFilter, setSourceFilter] = useState<SourceFilter>("all");
  const [selectedTopics, setSelectedTopics] = useState<Set<string>>(new Set(allTopicIds));
  const [started, setStarted] = useState(false);
  const [finished, setFinished] = useState(false);
  const [index, setIndex] = useState(0);
  const [answers, setAnswers] = useState<(CommSkillsAnswerRecord | null)[]>([]);

  const questions = useMemo(() => {
    return allBank.filter((q) => (sourceFilter === "all" || q.source === sourceFilter) && selectedTopics.has(q.topic));
  }, [sourceFilter, selectedTopics]);
  ...
  function handleAnswered(record: CommSkillsAnswerRecord) {
    setAnswers((prev) => {
      const next = [...prev];
      next[index] = record;
      return next;
    ...
    return (
      <div className="space-y-6">
        <div className="rounded-2xl border border-card-border bg-card p-5 sm:p-6 space-y-4">
          <p className="text-sm font-semibold text-heading">Choose a section</p>
          <div className="grid gap-2 sm:grid-cols-3">
            {sourceFilters.map((f) => {
              const count = allBank.filter((q) => f.id === "all" || q.source === f.id).length;
              ...
              return (
                <button
                  ...
                  className={`rounded-xl border-2 px-3 py-2.5 text-left text-sm transition-colors ${
                    active ? "border-accent bg-accent/10" : "border-card-border hover:border-accent"
                  }`}
                >
                  <span className="block font-medium text-heading">{f.label}</span>
                  <span className="text-xs text-secondary">{count} questions</span>
                </button>
              );
            })}
          </div>
        ...
        <div className="rounded-2xl border border-card-border bg-card p-5 sm:p-6 space-y-4">
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            {commSkillsTopics.map((t) => {
              const count = allBank.filter((q) => (sourceFilter === "all" || q.source === sourceFilter) && q.topic === t.id).length;
              return (
                <label
                  key={t.id}
          ...
          <p className="text-xs text-secondary">
            Section A preserves every past-exam question exactly as it appeared in the source, including the
            lecturer&rsquo;s own wording. Section B is a comprehensive concept bank built from the lecture slides,
            covering the same topics from multiple angles.
          </p>
          <button
            type="button"
            onClick={start}
  ...
  if (finished) {
    const answered = answers.filter((a): a is CommSkillsAnswerRecord => a !== null);
      ...
      <div className="rounded-2xl border border-card-border bg-card p-6 text-center space-y-4">
        ...
        <p className="text-sm text-body">
          {pct >= 80
            ...
            : "This section needs more practice — re-read the relevant material, then retry."}
        </p>
  ...
  const topicLabel = commSkillsTopics.find((t) => t.id === question.topic)?.label ?? question.topic;
  ...
  return (
    <div className="space-y-4">
      ...
      <CommSkillsQuestionCard key={question.id} question={question} savedAnswer={answers[index]} onAnswered={handleAnswered} />
